    ...
    <div id="arweave" uk-modal="" class="uk-modal" style="" tabindex="0">
      <div class="uk-modal-dialog uk-modal-body">
        ...
        <div v-for="transaction in transactions" :key="transaction">
          <hr />
          <a :href="'https://viewblock.io/arweave/tx/' + transaction.node.id">
            {{ transaction.node.id }}</a
          >
        </div>
        <hr />
        <form>
...
export default {
  ...
  methods: {
    connectAr: async function () {
      var wt = await window.arweaveWallet.connect(
        [
          "ACCESS_ADDRESS",
        ...
        { name: "Bridge-Market", logo: "https://verto.exchange/logo_dark.svg" }
      );
